...
plot(fblog, vertex.label=NA, vertex.size=6)
...
# To calculate p-values with statistical significant condition
sum(corr.pvals < 0.05)
sum(corr.pvals.adj < 0.05)
...
# To calculate p-values with no condition
sum(corr.pvals)
sum(corr.pvals.adj)

# ---
## [1] 2309.082 - Without Adj
## [1] 2805.727 - Adj
# ---

# We can see with the summary command also that median and mean values of p-values increased after adjustment
summary(corr.pvals)
...
# Question 4
length(E(graph.intersection(g.regDB, g.huge, byname=FALSE)))
...
g.huge<-upgrade_graph(g.huge)

plot(g.huge, vertex.label=NA, vertex.size=6)
# ---
## IGRAPH U--- 153 759 --
# ---
...
wut = graph.intersection(g.regDB, g.huge, byname=FALSE)
